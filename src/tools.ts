   function cutParams(params:any):string[] {
    ...
    function showlink(document:any,multiple=false):string[] {
        ...
        if(!multiple){
            ...
            if(document.getElementById("nometrics").checked)url=url+"&nometrics=True";
            if(document.getElementById("property").checked)url=url+"&property=True";
    ...
    export function hourglass(document:any,show=true){
        if(show)
     ...
     */
    export function postFile(document:any,_public:boolean=false){
     ...
     */
    function runAll(url:string,document:any){
    ...
    var hLog:any=null;
    function showLog(){
        if(hLog!=null)clearInterval(hLog);
        hLog=setInterval(()=>{
            window.open("/log","out");
        },10000);
    }
